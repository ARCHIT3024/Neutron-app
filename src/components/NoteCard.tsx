...
export default function NoteCard({ 
  ...
  // Basic Markdown to HTML (simplified for display)
  const renderContent = (markdown: string) => {
    ...
    // Ensure list items are handled correctly even if they are the first thing or follow other elements
    html = html.replace(/^(\*|-) (.*?)(?=<br \/>|$)/gm, (match, bullet, item) => `<li>${item.trim()}</li>`);
    html = html.replace(/(<li>.*?<\/li>)+/gs, '<ul>$1</ul>'); // Wrap LIs in UL
    // Remove <br /> inside <li> generated by mistake
    html = html.replace(/<li>(.*?)<br \/>(.*?)<\/li>/g, '<li>$1$2</li>');
    // Ensure paragraphs are correctly handled if not already wrapped
    if (!html.startsWith('<p>') && !html.startsWith('<ul>')) {
      html = `<p>${html}</p>`;
    }
    html = html.replace(/<\/p><p>/g, '</p><p class="mt-2">'); // Add margin between paragraphs
    html = html.replace(/<\/ul><p>/g, '</ul><p class="mt-2">'); // Add margin after list
    ...
    >
      ...
      >
        <div 
          ...
          // Make title clickable to edit if not view only. But Card click already handles this.
          // onClick={!isViewOnly ? (e) => { e.stopPropagation(); handleEditClick(); } : undefined} 
        >
          <h3 
      ...
      <CardContent 
        ...
        aria-label={`Note content: ${note.content.substring(0,100)}${note.content.length > 100 ? '...' : '' }`}
        // Make content clickable to edit if not view only. But Card click already handles this.
        // onClick={!isViewOnly ? (e) => { e.stopPropagation(); handleEditClick(); } : undefined}
      >
        ...
        {note.imageUrl && (
          <div className="mt-2 aspect-video relative overflow-hidden rounded-md">
            <Image 
              ...
              priority={note.isPinned} // Prioritize pinned notes' images, or first few notes
            />
          </div>
        )}
      </CardContent>
