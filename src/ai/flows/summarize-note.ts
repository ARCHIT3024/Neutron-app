...
const SummarizeNoteInputSchema = z.object({
  noteContent: z.string().describe('The content of the sticky note to summarize.'),
});
export type SummarizeNoteInput = z.infer<typeof SummarizeNoteInputSchema>;

const SummarizeNoteOutputSchema = z.object({
  summary: z.string().describe('A concise summary of the sticky note content.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a helpful assistant that summarizes sticky notes.

  Summarize the following sticky note content:
  {{noteContent}}`,
});
